...
class greedyred(BaseLogic):
    ...
    def heuristic(self, pos1: Position, pos2: Position, board: Board = None) -> int:
        """
        Estimasi jarak Manhattan dikurangi weight objek pada posisi tujuan.
        Memperhitungkan teleporter jika ada.
        Blue diamond: 1
        Red diamond: 2
        Red button: 1 (atau 4 jika diamond sekitar base <= 2)
        """
    ...
    def next_move(self, board_bot: GameObject, board: Board) -> Tuple[int, int]:
        ...
        # Filter game objects berdasarkan tipe
        game_objects = board.game_objects
        red_buttons = [d for d in game_objects if d.type == "DiamondButtonGameObject"]
        diamonds = [d for d in game_objects if d.type == "DiamondGameObject"]
        blue_diamonds = [d for d in diamonds if getattr(d.properties, "points", 1) == 1]
        red_diamonds = [d for d in diamonds if getattr(d.properties, "points", 1) == 2]

        # Cek diamond di radius 4 dari base
        nearby_diamonds = [
            d for d in diamonds
            if abs(d.position.x - base.x) + abs(d.position.y - base.y) <= 4
        ]

        # Strategi pemilihan target berdasarkan (jarak - weight):
        target = None
        
        # Consider all diamonds and buttons together, let heuristic handle weights
        available_targets = diamonds + red_buttons
        if available_targets:
            target = min(available_targets, key=lambda d: self.heuristic(d.position, base, board))
